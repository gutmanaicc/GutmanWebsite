...
- טוקנים (צבעים, גופנים, צללים) ב-`tailwind.config.js`. צבעי המותג: `#191919` דיו, `#FF2D85` ורוד (הטוקן `brand`). הקנבס הנוכחי כהה (`canvas: #0d0c11`).
- **שני ורודים עדיין חיים בקוד, וזה חוב ידוע.** הטוקן `brand` הוא `#FF2D85`, אבל `#ff5f9e` (הוורוד הישן, הרך יותר) עדיין יושב בכעשרה מקומות ב-`index.css` — גרדיאנטים, צללים ו-`.btn-brand` — וגם ב-`ProcessSection`. במקביל `#FF2D85` מקודד קשיח בכ-66 מקומות במקום להשתמש בטוקן. ההערה ב-`tailwind.config.js` מתארת את האיחוד כאילו הושלם; הוא לא. כשאתה נוגע בקובץ שיש בו הארדקוד, העבר אותו לטוקן, אבל אל תריץ החלפה גורפת בלי לבדוק — חלק מהמופעים הם `rgba()` בתוך גרדיאנטים וצללים שאין להם מקבילה ישירה בטוקן.
