...
- אינטראקציות בסגנון אפל (הסקיל apple-design מותקן ב-`.agents/skills/`): פידבק ב-pointer-down, צניעות בתנועה, prefers-reduced-motion.
- הסקילים `apple-design` ו-`ui-ux-pro-max` כבר בתוך ה-repo תחת `.agents/skills/`. לקרוא את apple-design לפני כתיבת האינטראקציות.
