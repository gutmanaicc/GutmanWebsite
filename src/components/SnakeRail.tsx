 ...
 *
 * הגרסה הקודמת (SnakePath, הוסרה) פרשה את הכרטיסים לסירוגין משני צדי הקו,
...
const SnakeRail = ({ steps, id }: Props) => {
   ...
   * הנגישות (a11y-reduce-motion) יעצור גם הוא את התנועה. הגרסה הקודמת
   * הכירה רק את העדפת המערכת והמשיכה לרוץ אחרי לחיצה בתפריט.
   */
  ...
  return (
    <div
       ...
       * שהבלוק עצמו היה ממורכז בדיוק.
       *
       * הרוחב נע עם המסך ולא קבוע, כי גודל הגופן של display-2 יורד
       * במסכים צרים והכותרת מתקצרת יחד איתו. רוחב קבוע החזיק את הרצועה
       * בתוך הכותרת ב-1280 ומעלה, אבל הוציא אותה החוצה ב-1024 (22px)
       * וב-860 (62px) - בדיוק הבעיה שרצינו לפתור, רק במסך אחר.
       */
      className="relative mx-auto w-full max-w-[clamp(26rem,52vw,40rem)]"
      data-snake={id}
    >
      {d && (
        <svg
