...
type Props = {
  ...
  onCta?: () => void;
  /*
   * מסתיר את הבקשה שבתחתית הקרוסלה.
   *
   * בדף הנחיתה יושבת מתחתיה סדרת תמונות, ובקשה שנופלת בין הסרטונים
...
const StudentWorksCarousel = ({
  ...
  title,
  /*
   * "ממסלולי האקדמיה" ולא "במהלך המסלול".
   *
   * חלק מהתוצרים מתויגים ליותר מקורס אחד ב-courseSlugs (למשל סרטון
   * ממסלול הווידאו שמוצג גם בעמוד הסושיאל, כי הוא תוכן ממותג שרלוונטי
   * לשניהם). "במהלך המסלול" ביידוע היה טוען שהוא נוצר דווקא בקורס
   * שהעמוד הנוכחי מציג - נכון לחלק מהעמודים ושקרי בשאר. הניסוח הכללי
   * נכון בכל עמוד שהקרוסלה תופיע בו, בלי תלות באיזה courseSlug הביא
   * אליה את הביקור.
   */
  sub = "סרטונים אמיתיים ממסלולי האקדמיה - לא הדגמות מבוימות.",
  note = "כל אחד מהסרטונים האלה נבנה באחד ממסלולי האקדמיה, על ידי מישהו שהתחיל מאפס.",
  ...
  const onPointerUp = (e: React.PointerEvent) => {
    ...
    /*
     * התוכן הולך עם האצבע, כמו בכל קרוסלה במובייל.
     *
     * קודם היה הפוך: החלקה ימינה הזיזה את הרצועה שמאלה, כי הכיוון נגזר
     * מסדר ה-RTL של הרשימה ולא מהתנועה הפיזית. גרירה ימינה חייבת להזיז
     * את התוכן ימינה, כלומר להביא למרכז את הכרטיס שמשמאל - וב-RTL זה
     * index + 1.
     */
    if (dx > 0) goNext();
    else goPrev();
