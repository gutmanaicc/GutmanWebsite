...
const AccessibilityWidget = () => {
  ...
  return (
    <>
      <button
        type="button"
        className="a11y-launcher fixed bottom-4 left-4 z-50 flex h-11 w-11 items-center justify-center rounded-full border border-zinc-200 bg-white text-[#121212] shadow-[0_6px_20px_-6px_rgba(18,18,18,0.28)] transition-[transform,box-shadow,background-color,color] duration-200 ease-out hover:scale-105 hover:border-[#FF2D85]/50 hover:text-[#FF2D85] hover:shadow-[0_12px_28px_-8px_rgba(255,45,133,0.35)] focus:outline-none focus-visible:scale-105 focus-visible:ring-2 focus-visible:ring-[#FF2D85] focus-visible:ring-offset-2 focus-visible:ring-offset-canvas active:scale-100"
        onClick={() => setOpen((v) => !v)}
      ...
      {open && (
        <div id="a11y-panel" role="dialog" aria-label="תפריט נגישות" className="a11y-panel" dir="rtl">
          ...
          <div className="a11y-font-row">
            <button type="button" onClick={() => update({ fontStep: Math.max(FONT_MIN, prefs.fontStep - 1) })} disabled={prefs.fontStep <= FONT_MIN} aria-label="הקטן גופן">−</button>
            <span aria-live="polite">{100 + prefs.fontStep * FONT_STEP_PCT}%</span>
            <button type="button" onClick={() => update({ fontStep: Math.min(FONT_MAX, prefs.fontStep + 1) })} disabled={prefs.fontStep >= FONT_MAX} aria-label="הגדל גופן">+</button>
          </div>
          ...
          <button type="button" className="a11y-statement-link" onClick={() => setStatementOpen(true)}>הצהרת נגישות מלאה</button>
        </div>
      )}

      {statementOpen && (
        <div className="a11y-modal-backdrop" onClick={() => setStatementOpen(false)}>
          <div className="a11y-modal" role="dialog" aria-label="הצהרת נגישות" dir="rtl" onClick={(e) => e.stopPropagation()}>
            <div className="a11y-head">
              <h2>הצהרת נגישות</h2>
            ...
            <div className="a11y-statement">
              <p>אתר gutmanai.com של Gutman, האקדמיה הרשמית ל-AI, שם דגש על הנגשת האתר לכלל הגולשים, לרבות אנשים עם מוגבלות.</p>
              <h3>רכז הנגישות</h3>
              <p>
                שם: רון גוטמן<br />
                טלפון: <a href="tel:+972508185107" dir="ltr">050-8185107</a><br />
